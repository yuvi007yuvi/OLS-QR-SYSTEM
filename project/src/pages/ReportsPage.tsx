...
import { Search, Filter, Download, Eye, Calendar, CheckCircle, XCircle } from 'lucide-react';
...
const ReportsPage: React.FC = () => {
  ...
  const [filterStatus, setFilterStatus] = useState<'all' | 'pending' | 'before-only' | 'complete'>('all');
  ...
  useEffect(() => {
    ...
    if (startDate && endDate) {
      ...
      filtered = filtered.filter(location => {
        ...
        if (location.createdAt instanceof Date) {
          createdAtDate = location.createdAt;
        } else if (typeof location.createdAt === 'string') {
          createdAtDate = new Date(location.createdAt);
        } else if (location.createdAt && typeof location.createdAt.toDate === 'function') {
          createdAtDate = location.createdAt.toDate(); // Assuming it's a Firebase Timestamp
        } else {
          // Fallback for unexpected types, perhaps log an error or handle as needed
          return false; // Exclude if createdAt cannot be parsed
        }
  ...
  }, [locations, searchTerm, filterStatus, startDate, endDate]);
  ...
  const exportToCSV = () => {
    const headers = ['QR Code ID', 'Location', 'Area', 'Supervisor', 'Contact', 'Status', 'Before Photo', 'After Photo', 'Created Date'];
    const csvData = filteredLocations.map(location => [
      ...
      location.createdAt instanceof Date ? location.createdAt.toLocaleDateString() : (location.createdAt as any).toDate().toLocaleDateString()
    ]);
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">Monitor photo upload status and manage location documentation</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <div className="text-2xl font-bold text-green-600">
              {filteredLocations.filter(l => l.beforePhoto && l.afterPhoto).length}
            </div>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredLocations.map((location) => (
                  <tr key={location.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">{location.supervisorName}</div>
                      <div className="text-sm text-gray-500">{location.contactNumber}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      <div className="flex space-x-4">
                        <span className={`flex items-center ${location.beforePhoto ? 'text-green-600' : 'text-gray-400'}`}>
                          {location.beforePhoto ? <CheckCircle className="h-4 w-4 mr-1" /> : <XCircle className="h-4 w-4 mr-1" />}
                          Before
                        </span>
                        <span className={`flex items-center ${location.afterPhoto ? 'text-green-600' : 'text-gray-400'}`}>
                          {location.afterPhoto ? <CheckCircle className="h-4 w-4 mr-1" /> : <XCircle className="h-4 w-4 mr-1" />}
                          After
                        </span>
                      </div>
                    </td>
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-900">{selectedLocation.contactNumber}</p>
                  </div>
                </div>
